...
@objc class DataDownloader: BaseOperation {
    ...
    // Initialize operation for events List
    ...
    override func main() {
        ...
        NSLog("Initiating Download for URL: \(String(describing: self.urlToDownloadData?.absoluteString))")
        ...
        let task = DataDownloader.session.dataTask(with: request as URLRequest) { (data, response, error) in
            ...
            if(error != nil) {
                ...
                // http error code is not the success one
